...
export function useFaceYogaPipeline({
  ...
  const resolvedRepTarget =
    repTarget && repTarget > 0 ? repTarget : undefined;
  ...
  }, [active, isFaceMode, isFaceHand, stopFaceLandmarker, stopHandLandmarker]);
  ...
  useEffect(() => {
    if (!active || !isFaceHand || !cameraReady) {
      if (!isFaceHand) stopHandLandmarker();
      return;
    }
    const v = videoRef.current;
    if (!v) return;
